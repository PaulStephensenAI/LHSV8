...
const ragDbPath = path.resolve(process.cwd(), 'storage', 'lavenderhill_rag.db');
...
// Helper to analyze Toni persona formatting
function analyzeToniFormattingMetrics(text: string, companionId: string, scannable: boolean) {
  ...
  if (boldHeaders >= 2) score += 15;
  if (chunkCount >= 3 && chunkCount <= 14) score += 5;
  score = Math.min(100, Math.max(50, score));

  let formattingType: 'scannable_chunks' | 'bullet_scaffolding' | 'standard_pacing' = 'standard_pacing';
  if (scannable || companionId === 'toni' || companionId === 'elysian') {
    formattingType = hasBulletPoints ? 'bullet_scaffolding' : 'scannable_chunks';
  }

  return {
    applied: scannable || companionId === 'toni' || companionId === 'elysian',
    chunkCount,
    hasBulletPoints,
...
2. Scannability Guardrail: Alleviate research anxiety and cognitive fatigue with structured **bold keys** and scannable milestones when planning projects.
3. Greeting Pacing: For simple greetings, respond warmly and naturally without overwhelming with heavy milestone templates.
${scannable ? 'CRITICAL: Format response in structured bullet blocks with **Bold Key Terms** at the beginning of each item.' : ''}`;
...
function getStudioFallbackReply(companionId: string, message: string, scannable: boolean): string {
  ...
  // Keyword Deterministic Fallbacks from File 06
...
app.post('/api/chat/stream', async (req, res) => {
  ...
  try {
    ...
    if (streamSuccess) {
      ...
      const fallbackReply = getStudioFallbackReply(companionId, message, scannableMode);
    ...
    // Stage 5: Toni Persona Formatting Validation
    const toniFormatting = analyzeToniFormattingMetrics(fullText, companionId, scannableMode);
    events.push({
      stage: 'toni_formatting',
      status: 'success',
      timestamp: new Date().toISOString(),
      durationMs: 2,
      summary: `Toni Persona formatting verified (Scannability score: ${toniFormatting.scannabilityScore}%, ${toniFormatting.chunkCount} visual blocks)`,
      details: toniFormatting
    });
    ...
    res.write(`data: ${JSON.stringify({
      ...
      reply: fullText,
      companionId,
      isScannableFormatted: scannableMode || companionId === 'toni',
      metadata: {
        traceId,
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!replyText) {
      replyText = getStudioFallbackReply(companionId, message, scannableMode);
    }
